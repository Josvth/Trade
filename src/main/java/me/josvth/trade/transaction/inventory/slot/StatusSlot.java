...
public class StatusSlot extends Slot{
	...
	public void update(TransactionHolder holder) {
		if (holder.getOtherTrader().hasAccepted()) {
			setSlot(holder, ItemStackUtils.argument(acceptedItem, "%player%", holder.getOtherTrader().getName()));
		} else {
			setSlot(holder, ItemStackUtils.argument(consideringItem,  holder.getOtherTrader().getName()));
		}
	}
